...
function editdriverprofile(code)
{
var divid='edit'+code;
	if (code=='')
  		{
		  ...
		  {// code for IE6, IE5
			...
			xmlhttp.onreadystatechange=function()
 			 ...
 			 if (xmlhttp.readyState==4 && xmlhttp.status==200)
				...
				xmlhttp.open('GET','transport_apps_profile_editajax.php?profileid='+code,true);
				xmlhttp.send();
...
var divid="otherpassnumdiv";
	...
	if (window.XMLHttpRequest)
		...
		<?php echo "<form action='transport_apps.php?ecode=$viewid&type=$_GET[type]' method='post' accept-charset='utf-8' id='myform' class='formular' enctype='multipart/form-data' >";?>
		<?php
			...
			$sql="select code, type, name, phonenum, address from sources where id='$viewid'";
			$res=mysql_query($sql) or die(mysql_error());
			...
			<br/> <a href='company_apps_edit.php?editid=$responsibleid&ecode=$viewid&type=2'><img style='height:75px; width:60px;' src='appphotos/$responsibleid".".jpg"."?".time()."'/></a> رئیس مرکز: $responsiblename>>>>";
			if($rowvice['id']!=""){echo "<a href='company_apps_edit.php?editid=$rowvice[id]&ecode=$viewid&type=2' ><img style='height:75px; width:60px;margin-right:150px' src='appphotos/$rowvice[id]".".jpg"."?".time()."'/></a> معاون: $rowvice[name]>>></span>";
			}
			}else{
		...
		?>
			<fieldset>
				<legend align="right"><span dir="rtl">نام مرکز: <?php echo $roo['name'];?>>> شماره ثبت: <?php echo $roo['code'];?> >> شماره تماس: <?php echo $roo['phonenum'];?> >> آدرس: <?php echo $roo['address'];?></span></legend>
			<div id="tabs" align="right" style='font-family:tahoma;' >
				...
				<ul>
					...
					<?php 
					echo "<li><a href='#tabs-5'>فایل راننده ها</a></li>";
					
					
					echo "<li><a href='#tabs-6'>سوابق شرکت</a></li>";
					echo "<li><a href='#tabs-7'>طرف قرارداد ایرانی</a></li>";
				...
				<div id="tabs-1">
						...
						<input type="submit" class="submit"  style="border-top-left-radius: 10px ;border-top-right-radius: 10px ; border-bottom-left-radius: 10px ;border-bottom-right-radius: 10px ; cursor:pointer;color:white;  background-color:#5e6062; border:0;height:30px; width:100px;"  onmouseover="this.style.background='#252527'" onmouseout="this.style.background='#5e6062'" name="insertapplicant" value="ذخیره" id="linkdatabutton"/>
						
						<?php echo " <input type='button' id='cancel_cash' onclick='window.open (\"transport_apps.php?ecode=$viewid&type=$_GET[type]\",\"_parent\")' name='cancel' style='border-top-left-radius: 10px ;border-top-right-radius: 10px ; border-bottom-left-radius: 10px ;border-bottom-right-radius: 10px ; cursor:pointer;color:white;  background-color:#5e6062; border:0;height:30px; width:100px;'  onmouseover='this.style.background=\"#252527\"' onmouseout='this.style.background=\"#5e6062\"' name='cancel' value='انصراف'/></td>";?>
					...
					if(isset($_POST['insertapplicant'])){
					   ...
					   $image->save($destination);
						header("location:transport_apps.php?ecode=$viewid&type=$_GET[type]");
						die();
						//}
						}
					?>
				...
				<?php
					if(isset($_POST['insertstatus'])){
						...
						mysql_query($sql) or die (mysql_error());
						header("location:transport_apps.php?ecode=$viewid&type=$_GET[type]#tabs-1");
						die();
					}
				?>
				...
				<?php
				$i=1;
				$sql="select * from vapplicants where source_id='$viewid' and (responsibility='boss' || responsibility='vicep') and type!='driver'";
				$res=mysql_query($sql) or die(mysql_error());
				while($row=mysql_fetch_array($res)){
					...
					if($countdependant==0){
					echo"<td><a href='transport_apps.php?delid=$row[id]&ecode=$viewid&type=$_GET[type]' class='confirm'><img src='img/cancel.png' title='پاک کردن درخواست کننده'/></a>";
					}else{
						echo "<td><img src='img/cancel1.png'/>";
					}
						echo "<a href='company_apps_edit.php?editid=$row[id]&ecode=$viewid&type=2'><img src='img/edit.png' title='ویرایش درخواست کننده'/></a>					</tr>
					";
					$i++;
				...
				if(isset($_GET['editid'])){
					...
					header("location:transport_apps_edit.php?appid=$_GET[editid]&kid=$_GET[kid]");
					die();
				}
				...
				<a href="#" id="addlink_tab2" onclick="jQuery('#myform').validationEngine('hide')"><b>اضافه نمودن عملکرد جدید</b></a>
						...
						<tr align="right">
							...
							<input type="text"  class="validate[checkDuplicate[passportnum2],required]" onkeyup="checkpassnum(this.value,<?php echo $viewid;?>)" name="passnum1" id="passnum1"/>
							</td>
						</tr>
						<tr><td colspan='2'><br/></td></tr>
						<tr align="right">
				...
				<input type="button" style="background-color:white; border:0; background-image:url(img/cal.png); width:24px;height:24px " onclick="displayDatePicker('todate', this);"/>
						...
						<tr align="right">
							<td> عملکرد (واردات):</td>
							<td><input type="text" size="42" name="donejobs" value="0" id="donejobs"  class="validate[custom[integer]]"/> 
						...
						<tr align="right">
							<td> عملکرد (صادرات):</td>
							<td>
							<input type="text" size="42" name="donejobs_ex" value="0" id="donejobs_ex"  class="validate[custom[integer]]"/> 
					...
					<tr align="right">
						...
						<tr align="right">
							<td> نوع ویزا:</td>
							<td><select name="visatype" id="visatype" class="validate[required]" style="height:30px; width:210px">
						...
						<?php echo " <input type='button' id='cancel_cash' onclick='window.open (\"transport_apps.php?ecode=$viewid&type=$_GET[type]#tabs-2\",\"_parent\")' style='border-top-left-radius: 10px ;border-top-right-radius: 10px ; border-bottom-left-radius: 10px ;border-bottom-right-radius: 10px ; cursor:pointer;color:white;  background-color:#5e6062; border:0;height:30px; width:100px;'  onmouseover='this.style.background=\"#252527\"' onmouseout='this.style.background=\"#5e6062\"' name='cancel' value='انصراف'/></td>";?>
				...
				<?php
					if(isset ($_POST['insertdone'])){
						...
						mysql_query($sql) or die (mysql_error());
						header("location:transport_apps.php?ecode=$viewid&type=$_GET[type]#tabs-2");
						die();
					}
				...
				while($row=mysql_fetch_array($res)){
					...
					echo "<td><a href='resume_apps_edit.php?applicantid=$row[id]&editid=$rw[id]&ecode=$viewid&type=2'><img src='img/edit.png' title='ویرایش درخواست کننده'/></a>";

					echo"<a href='transport_apps.php?delresumeid=$row[id]&ecode=$viewid&type=$_GET[type]#tabs-2' class='confirm'><img src='img/cancel.png' title='پاک کردن درخواست کننده'/></a>
					</td></tr>";
				...
				<div id="tabs-3">
					<a href="#" id="addlink_tab3" onclick="jQuery('#myform').validationEngine('hide')"><b>اضافه نمودن ویزای سایرین</b></a>
						...
						<tr align="right">
							...
							<td align="right" >
							<input type="text" size="42" name="passnum"  id="passnum" onkeyup="checkpassnumothers(this.value,document.getElementById('depname').value)" class="validate[checkDuplicate[otherpassportnum2],required]" />
				</td>
						</tr>
						<tr><td colspan='2'><br/></td></tr>	
						<tr align="right">
							<td> تاریخ مراجعه: </td>
							<td>
							<input type="text" size="42" name="requestdate2"  id="requestdate2"  /> 
				<input type="button" style="background-color:white; border:0; background-image:url(img/cal.png); width:24px;height:24px " onclick="displayDatePicker('requestdate2', this);"/>
						...
						<tr align="right">
							<td width="20%"> نام متقاضی:</td>
							<td align="right" >
							<input type="text" size="42" name="depname"  id="depname" onkeyup="checkpassnumothers(document.getElementById('passnum').value,this.value)"  />
				</td>
						</tr>
						<tr><td colspan='2'><br/></td></tr>	
						<tr align="right">
					...
					<tr align="right">
						...
						<tr align="right">
							...
							<td>
							<select name="omarital_status" id="omarital_status" class="validate[required]">
								<option value="">انتخاب وضعیت تاهل</option>
								<option value="متاهل">متاهل</option>
								<option value="مجرد">مجرد</option>
							</select>
					...
					<tr align="right">
							...
							<td align="right" >
							<input type="text" size="42" name="dependentamount"  id="dependentamount" class="" />
				</td>
						</tr>
						<tr><td colspan='2'><br/></td></tr>
						<tr align="right">
							<td> نوع ویزا:</td>
							<td>
							<select name="ovisatype" id="ovisatype" class="validate[required]" style="height:30px; width:210px">
								<option value="">انتخاب نوع ویزا</option>
								<?php
									$sql="select * from visatypes";
									$retype=mysql_query($sql) or die (mysql_error());
						...
						<tr align="right">
							<td> تاریخ صدور گذرنامه:</td>
							<td>
							<input type="text" size="42" name="opassissuedate"  id="opassissuedate"  /> 
							</td>
						</tr>
						<tr><td colspan='2'><br/></td></tr>
						<tr align="right">
							<td> محل صدور گذرنامه:</td>
							<td>
							<input type="text" size="42" name="opassissueplace"  id="opassissueplace"  /> 
							</td>
						</tr>
						<tr><td colspan='2'><br/></td></tr>
						...
						<tr align="right">
							<td> دستور رسیدگی:</td>
							<td>
							<input type="text" size="42" name="ovisaorder"  id="ovisaorder"  /> 
							</td>
						</tr>
						<tr><td colspan='2'><br/></td></tr>
						<tr align="right">
							<td>مجوز مرکز (شماره):</td>
							<td>
							<input type="text" size="42" name="ocenternum"  id="ocenternum"  /> 
							</td>
						</tr>
						<tr><td colspan='2'><br/></td></tr>
						<tr align="right">
							<td> مجوز مرکز (تاریخ):</td>
							<td>
							<input type="text" size="42" name="ocenterdate"  id="ocenterdate"  /> 
							</td>
						</tr>
						<tr><td colspan='2'><br/></td></tr>
						<tr align="right">
							<td> ارجعیت:</td>
							<td>
							<input type="text" size="42" name="oarjaeet"  id="oarjaeet"  /> 
							</td>
						</tr>
						<tr><td colspan='2'><br/></td></tr>
						<tr align="right">
							<td> شماره روادید صادره:</td>
							<td>
							<input type="text" size="42" name="ovisanum"  id="ovisanum"  /> 
							</td>
						</tr>
						<tr><td colspan='2'><br/></td></tr>
						<tr align="right">
							<td> تاریخ صدور روادید:</td>
							<td>
							<input type="text" size="42" name="ovisaissuedate"  id="ovisaissuedate"  /> 
					...
					<tr align="right">
							...
							<td align="right" >
							<input type="text" size="42" name="visaprice"  id="visaprice" class="" />
				</td>
						</tr>
							<tr><td colspan='2'><br/></td></tr>	
					<tr align="right">
							...
							<td align="right" >
							<input type="text" size="42" name="limitdays"  id="limitdays" class="" />
				</td>
						</tr>
				...
				<?php
					if(isset ($_POST['insertdep'])){
						$sql="insert into dependent_visa (source_id, fromdate, name, father_name, supervisor, relation, dependentamount, passportnum, visa_type, visaprice, limitdays,
						iranaddress, afghanistanadd, birthdate, birthplace, marital_status, education, passissuedate, passissueplace, attachtype, visaorder, centernum, centerdate, arjaeet, visanum, visadate, details, requestdate) 
						values('$viewid', '$_POST[fromdatedep]', '$_POST[depname]', '$_POST[depfname]', '$_SESSION[user_id]', '$_POST[relation]', '$_POST[dependentamount]', '$_POST[passnum]', '$_POST[ovisatype]', '$_POST[visaprice]', '$_POST[limitdays]', 
						'$_POST[oiraddress]', '$_POST[oafghanistanadd]', '$_POST[obdate]', '$_POST[obplace]',
						'$_POST[omarital_status]', '$_POST[oeducation]', '$_POST[opassissuedate]', '$_POST[opassissueplace]', '$_POST[orequesttype]', '$_POST[ovisaorder]', 
						'$_POST[ocenternum]', '$_POST[ocenterdate]', '$_POST[oarjaeet]','$_POST[ovisanum]', '$_POST[ovisaissuedate]', '$_POST[details]', '$_POST[requestdate2]')";
						mysql_query($sql) or die (mysql_error());
				...
				while($row=mysql_fetch_array($res)){
					...
					if($row['back']==1){
						...
						echo "<td><a style='color:blue' href='transport_apps.php?accept_dependent=1&ecode=$viewid&type=$_GET[type]&backdep=$row[id]#tabs-3' >تایید بازگشت</a></td>";
					}
					echo "<td><a href='company_apps_others_edit.php?editid=$row[id]&ecode=$viewid&type=2'><img src='img/edit.png' title='ویرایش درخواست کننده'/></a>
					
					";
					echo"<a href='transport_apps.php?deliddep=$row[id]&ecode=$viewid&type=$_GET[type]#tabs-3' class='confirm'><img src='img/cancel.png' title='پاک کردن درخواست کننده'/></a>
					</td></tr>";
				...
				<?php
					if(isset($_POST['insertattach'])){
					   ...
					   $image->save($destination);
						...
						header("location:transport_apps.php?ecode=$viewid#tabs-4");
					die();
					}
				...
				$res=mysql_query($sql) or die(mysql_error());
				while($row=mysql_fetch_array($res)){
					//<td> <a href='source-attachments/$viewid/".$row['id'].$row['format']."'>دانلود</a></td>
					echo "
					<tr align='center'>
					<td>$i</td>
					...
					echo"
					<td><a href='transport_apps.php?delattach=$row[id]&ecode=$viewid&ftype=$row[format]' class='confirm'><img src='img/cancel.png'/></a></td>";
				...
				<div id="tabs-5">
				<a href="#" id="addlink_tab4" onclick="jQuery('#myform').validationEngine('hide')"><b>اضافه نمودن فایل جدید</b></a>
						<a href="#" id="addlinkback_tab4" onclick="jQuery('#myform').validationEngine('hide')"><b>اضافه نمودن فایل جدید</b></a>
						<input type="button" style=" width:20px; background-color:white; border:0;background-image:url(img/back.png)" /></br>
						<hr align="right"style=" width:150px"/>
						
						<div id="addlinkdata_tab4" >
						<table width="50%">
							<tr>
								<td>شماره فایل</td><td><input type="text" name="profile_num" id="profile_num" class="validate[required]"/></td>
							</tr>
							<tr>
							<td>تاریخ ایجاد</td>
							<td align="right" >
							<input type="text" name="create_date" id="create_date" class="validate[required,custom[date]]" style="width:200px" />
				<input type="button" style="background-color:white; border:0; background-image:url(img/cal.png); width:24px;height:24px " onclick="displayDatePicker('create_date', this);"/>
							...
							<br/>
						<input type="submit" class="submit"  name="insertprofile" style="border-top-left-radius: 10px ;border-top-right-radius: 10px ; border-bottom-left-radius: 10px ;border-bottom-right-radius: 10px ; cursor:pointer;color:white;  background-color:#5e6062; border:0;height:30px; width:100px;margin-right:60px"  onmouseover="this.style.background='#252527'" onmouseout="this.style.background='#5e6062'" value="ذخیره" id="linkdatabutton_tab4"/>
						
						<?php echo " <input type='button' id='cancel_cash' onclick='window.open (\"transport_apps.php?ecode=$viewid&type=$_GET[type]#tabs-5\",\"_parent\")' style='border-top-left-radius: 10px ;border-top-right-radius: 10px ; border-bottom-left-radius: 10px ;border-bottom-right-radius: 10px ; cursor:pointer;color:white;  background-color:#5e6062; border:0;height:30px; width:100px;'  onmouseover='this.style.background=\"#252527\"' onmouseout='this.style.background=\"#5e6062\"' name='cancel' value='انصراف'/></td>";?>
				<?php
					if($_POST["insertprofile"]){
						$sql="insert into driver_profiles (profile_num, source_id, created_at) values ('$_POST[profile_num]', '$viewid', '$_POST[create_date]')";
						mysql_query($sql) or die (mysql_error());
						$sql="select id from driver_profiles order by id desc LIMIT 1";
							$rscenter=mysql_query($sql) or die (mysql_error());
							$rwcenter=mysql_fetch_array($rscenter);
						mkdir("driver-attachments/$rwcenter[id]");
						header("location:transport_apps.php?ecode=$viewid&type=$_GET[type]#tabs-5");
						die();
					}
				...
				<table width="50%" id="table">
				<tr>
				<th>شماره فایل راننده</th>
				<th>مشخصات راننده</th>
				<th>تاریخ ایجاد</th>
				<th>نمایش فایل راننده</th>
				<th>عملیات</th>
				</tr>
				<?php
				$i=1;
				//may need changes
				$sql="select * from driver_profiles where source_id='$viewid'  order by profile_num asc";
				$res=mysql_query($sql) or die(mysql_error());
				while($row=mysql_fetch_array($res)){
					
					echo "
					<tr align='center'>
					<td>$row[profile_num]</td>";
					//Getting the name of the driver
					$sqdname="select name, father_name from vapplicants where  profile_num='$row[id]' and responsibility='new'";
					$rsdname=mysql_query($sqdname) or die (mysql_error());
					$rwdname=mysql_fetch_array($rsdname);
					
					echo"
					<td>$rwdname[name] فرزند $rwdname[father_name]</td>
					<td>$row[created_at]</td>
					<td><a style='color:blue' href='driverprofile.php?pid=$row[id]'>نمایش پروفایل</a></td>
					";
					$sq="select count(*) from vapplicants where profile_num='$row[id]'";
					$rscount=mysql_query($sq) or die (mysql_error());
					$rwcount=mysql_fetch_array($rscount);
					
					if($rwcount['count(*)']==0){
					echo"<td><a  href='transport_apps.php?deletep=$row[id]&ecode=$viewid&type=$_GET[type]#tabs-5' class='confirm'><img src='img/cancel.png' title='پاک کردن درخواست کننده'/></a>";
					}else{
						echo "<td><img src='img/cancel1.png'/>";
					}
					echo "<img src='img/edit.png' onclick='editdriverprofile($row[id])' title='ویرایش پروفایل'/></a>
										
					
					</tr>
					<tr><td id='edit$row[id]' colspan='5'></td></tr>
					";
					$i++;
				...
				if(isset($_GET['deletep'])){
					$sq="delete from driver_profiles where id='$_GET[deletep]'";
					mysql_query($sq) or die (mysql_error());
					header("location:transport_apps.php?ecode=$viewid&type=$_GET[type]#tabs-5");
					die();
				...
				if(isset($_POST['updateprofile'])){
					$sql="update driver_profiles set profile_num='$_POST[nprofile_num]', created_at='$_POST[ncreate_date]' where id='$_POST[profileid]'";
					mysql_query($sql) or die (mysql_error());
					header("location:transport_apps.php?ecode=$_POST[sourceid]#tabs-5");
					die();
				}
				if(isset($_POST['cupdateprofile'])){
					header("location:transport_apps.php?ecode=$_POST[sourceid]#tabs-5");
					die();
				}
				?>
				...
				<?php
				$i=1;
				$sql="select * from vapplicants where source_id='$viewid' and (responsibility='oldboss' || responsibility='oldvicep')";
				$res=mysql_query($sql) or die(mysql_error());
				while($row=mysql_fetch_array($res)){
					if($row['responsibility']=="boss"){
						$responsibility="رئیس";
				...
				<div id="tabs-7">
				<a href="#" id="addlink_tab5" onclick="jQuery('#myform').validationEngine('hide')"><b>اضافه نمودن طرف قرارداد جدید</b></a>
						<a href="#" id="addlinkback_tab5" onclick="jQuery('#myform').validationEngine('hide')"><b>اضافه نمودن طرف قرارداد جدید</b></a>
						<input type="button" style=" width:20px; background-color:white; border:0;background-image:url(img/back.png)" /></br>
						<hr align="right"style=" width:150px"/>
						
						<div id="addlinkdata_tab5" >
						<table width="80%">
							<tr>
								<td>نام شرکت ایرانی</td><td><input type="text" name="coname" id="profile_num" class="validate[required]"/></td>
							
							<td>آدرس</td>
							<td align="right" width="50%" >
							<input type="text" name="coaddress" size='70' id="coaddress" class="validate[required]" />
				</td>
							</tr>
						</table>
							<br/>
						<input type="submit" class="submit"  name="insertco" style="border-top-left-radius: 10px ;border-top-right-radius: 10px ; border-bottom-left-radius: 10px ;border-bottom-right-radius: 10px ; cursor:pointer;color:white;  background-color:#5e6062; border:0;height:30px; width:100px;margin-right:60px"  onmouseover="this.style.background='#252527'" onmouseout="this.style.background='#5e6062'" value="ذخیره" id="linkdatabutton_tab5"/>
						
						<?php echo " <input type='button' id='cancel_cash' onclick='window.open (\"transport_apps.php?ecode=$viewid&type=$_GET[type]#tabs-7\",\"_parent\")' style='border-top-left-radius: 10px ;border-top-right-radius: 10px ; border-bottom-left-radius: 10px ;border-bottom-right-radius: 10px ; cursor:pointer;color:white;  background-color:#5e6062; border:0;height:30px; width:100px;'  onmouseover='this.style.background=\"#252527\"' onmouseout='this.style.background=\"#5e6062\"' name='cancel' value='انصراف'/></td>";?>
				<?php
					if($_POST["insertco"]){
						$sql="insert into iraniancompany (name, address, source_id) values ('$_POST[coname]', '$_POST[coaddress]', '$viewid')";
						mysql_query($sql) or die (mysql_error());
						header("location:transport_apps.php?ecode=$viewid&type=$_GET[type]#tabs-7");
						die();
					}
				...
				<?php
				$i=1;
				$sql="select * from iraniancompany where source_id='$viewid'  order by id desc";
				$res=mysql_query($sql) or die(mysql_error());
				while($row=mysql_fetch_array($res)){
					...
					echo"<td><a  href='transport_apps.php?deleteco=$row[id]&ecode=$viewid&type=$_GET[type]#tabs-5' class='confirm'><img src='img/cancel.png' title='پاک کردن درخواست کننده'/></a>
					</tr>
					";
					$i++;
				...
				if(isset($_GET['deleteco'])){
					$sq="delete from iraniancompany where id='$_GET[deleteco]'";
					mysql_query($sq) or die (mysql_error());
					header("location:transport_apps.php?ecode=$viewid&type=$_GET[type]#tabs-7");
					die();
